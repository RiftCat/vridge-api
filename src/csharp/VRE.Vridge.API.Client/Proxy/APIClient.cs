...
namespace VRE.Vridge.API.Client.Proxy
{
    public class APIClient
    {
        ...
        /// </summary>        
        public APIStatus GetStatus(int timeoutMs = 500)
        {
            APIStatus status;

            ConnectToControlSocket();

            controlSocket.SendAsJson(new ControlRequestHeader(appName, ControlRequestCode.RequestStatus));            
            var success = controlSocket.TryReceiveJson(out status, timeoutMs);
        ...
        public T CreateProxy<T>(bool keepAlive = true, int timeoutMs = 100)
        {            
            ...
            bool success = controlSocket.TrySendAsJson(new RequestEndpoint(endpointName, appName), timeoutMs);
            ...
            success = controlSocket.TryReceiveJson(out response, timeoutMs);
